...
static char network_map_file[256] = "/home/dhruv/winshare/Code/mosquitto/plugins/payload-modification/network_map.txt";
static char trust_store_template[256] = "/home/dhruv/winshare/Code/mosquitto/plugins/payload-modification/trust_store/trust_store_%s.txt";
...
static int callback_message(int event, void *event_data, void *userdata) 
{
    ...
    if (!is_local_origin) {
        ...
        cJSON *signer_item;
        cJSON_ArrayForEach(signer_item, S_item) {
            if (cJSON_IsString(signer_item)) {
                double path_trust = get_least_trustworthy_path_score(signer_item->valuestring, broker_id);
                if (path_trust >= LOCAL_THRESHOLD_THETA) {
                    plugin_log(MOSQ_LOG_INFO, "[TRUST] ✅ Path from '%s' is acceptable (%.3f >= %.3f). Accepting message.", 
                                signer_item->valuestring, path_trust, LOCAL_THRESHOLD_THETA);
                    is_accepted = true;
                    break;
                } else {
                    plugin_log(MOSQ_LOG_INFO, "[TRUST] ❌ Path from '%s' is not acceptable (%.3f < %.3f).", 
                                signer_item->valuestring, path_trust, LOCAL_THRESHOLD_THETA);
                }
            }
        }
        if (!is_accepted) {
            plugin_log(MOSQ_LOG_WARNING, "[TRUST] ❌ Dropping. No signer had a trustworthy path above threshold %.3f", LOCAL_THRESHOLD_THETA);
            cJSON_Delete(root); free(payload_copy); 
            return MOSQ_ERR_ACL_DENIED;
        }
    }
