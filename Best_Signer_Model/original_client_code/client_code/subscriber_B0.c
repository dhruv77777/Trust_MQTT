...
{
    // [MODIFIED] Get and print the current time with high precision
    struct timespec ts;
    ...
    printf("\n----------------------------------------\n");
    printf(">>>> Message received at: %s.%03ld\n", time_buf, ts.tv_nsec / 1000000);
...
int main(int argc, char *argv[]) {
    if (argc != 2) {
        fprintf(stderr, "Usage: %s <BrokerID>\n", argv[0]);
        return 1;
    }

    char *broker_id_str = argv[1];
    int base_port = 1883;
    int broker_num = atoi(broker_id_str + 1); // Get number from "B0", "B1", etc.
    int broker_port = base_port + broker_num;
    ...
    char client_id[64];
    snprintf(client_id, sizeof(client_id), "interactive_subscriber_%s", broker_id_str);
    mosq = mosquitto_new(client_id, true, NULL);
    
    mosquitto_connect_callback_set(mosq, on_connect);
    mosquitto_message_callback_set(mosq, on_message);

    rc = mosquitto_connect(mosq, "localhost", broker_port, 60);
    if (rc != MOSQ_ERR_SUCCESS) 
    {
        fprintf(stderr, "Could not connect to broker %s on port %d. Error code: %d\n", broker_id_str, broker_port, rc);
        return 1;
    }
